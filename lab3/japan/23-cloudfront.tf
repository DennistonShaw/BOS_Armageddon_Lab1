...
resource "aws_cloudfront_distribution" "edo_cf01" {
  ...
  ordered_cache_behavior {
    ...
    cache_policy_id = data.aws_cloudfront_cache_policy.edo_use_origin_cache_headers01.id

    origin_request_policy_id = data.aws_cloudfront_origin_request_policy.edo_orp_all_viewer_except_host01.id
  }
  ...
  ordered_cache_behavior {
    ...
    cache_policy_id          = aws_cloudfront_cache_policy.edo_cache_api_disabled01.id
    origin_request_policy_id = aws_cloudfront_origin_request_policy.edo_orp_api01.id
  }
  ...
  ordered_cache_behavior {
    ...
    cache_policy_id            = aws_cloudfront_cache_policy.edo_cache_static01.id
    origin_request_policy_id   = data.aws_cloudfront_origin_request_policy.edo_orp_all_viewer01.id # ← Now forwards Host header
    response_headers_policy_id = aws_cloudfront_response_headers_policy.edo_rsp_static01.id
  }
  ...
  viewer_certificate {
    acm_certificate_arn      = aws_acm_certificate.edo_acm_cert01.arn
